...
```{r}
sp_to_remove <- c('uid','bird','lizard_uid','people')
db_indep <- db_joint %>% filter(startsWith(Event,"1|"),
                    !Species%in%sp_to_remove) 
db_indep %>% select(class, Station, Species) %>% distinct() %>% 
  group_by(Station) %>% add_count() %>% ungroup() %>% 
  select(class,n) %>% distinct() %>% 
  ggplot(aes(class,n))+geom_boxplot()
```
...
data.frame(div_index) %>% rownames_to_column('Station') %>% left_join(station_data) %>% 
  ...
  ggplot(aes(class, div_index))+geom_boxplot()
```

The diversity was higher in the forest, and in plots with an intermediate amount of Balsa trees. The remaining reforestation plots had similar diversity. 

```{r nmds}
db_indep %>% group_by(Station,Species) %>% count() %>% 
  pivot_wider(names_from = Species, values_from = n, values_fill = 0) %>% 
  column_to_rownames('Station') %>% 
  vegan::metaMDS() -> nmds_spp
...
We can analyze the number further by looking at the different categories, and using the frequency per station as a more standard metric than the count alone.
```{r freq}
...
In contrast, pumas, raccoons (<i>Procyon spp.</i>), tapirs (<i>Tapirus bairdii</i>) and both species of monkey were only detected in the regeneration plots. In the case of the monkeys this likely has to do with the sparsity of trees, which forces them to move closer to the ground. Some species that were detected across all categories had higher detection frequencies at stations with lower coverage, like common opossums. We observed an opposite trend, whereby species were more often seen in the forest of in areas with higher coverage for tamanduas. 
For multiple other species there was no clear trend.

Let's look to see if there is any clear trend of frequencies. I will include only species that were detected across more than 2 categories.
```{r freq v cat plot}
...
db_indep %>% filter(Species%in%sp_to_plot) %>% 
  ...
  select(Species,class,freq) %>% 
  distinct() %>% 
  ggplot(aes(class,freq))+geom_boxplot()+
  scale_y_log10()+
  facet_wrap(~Species)+
...
prin_comps <- predict(veg_pca)[,1:2]
biplot(veg_pca)
```
